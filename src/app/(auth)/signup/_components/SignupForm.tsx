...
export function SignupForm({ dict }: { dict: Dict['auth'] }) {
  const [state, action, pending] = useActionState(signup, undefined)

  return (
    <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-8 shadow-2xl">
      <h2 className="text-xl font-semibold text-white mb-6">{dict.signup_title}</h2>
      ...
      <form action={action} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            required
            minLength={6}
            placeholder={dict.password_hint}
            className="w-full px-4 py-2.5 rounded-lg bg-white/10 border border-white/20 text-white placeholder-blue-300/50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
          />
        </div>
        ...
        >
          ...
          {pending ? dict.signup_pending : dict.signup_cta}
        </button>
      </form>
